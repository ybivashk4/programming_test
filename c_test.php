...
<body>
    ...
    <?php
        $n = pg_fetch_array(pg_query(pg_connect("host=localhost dbname=tests user=postgres password=123"), "SELECT COUNT (*) \"Задание\" FROM tst.\"Задания\" WHERE \"id темы\"=2"))[0];
        for ($i = 0; $i < $n; $i+=1) {
            $query = "SELECT \"Задание\" FROM tst.\"Задания\" WHERE \"id темы\"=2 LIMIT 1 OFFSET $i";
